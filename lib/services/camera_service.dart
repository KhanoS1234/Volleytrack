...
class CameraService {
  ...
  // One PoseAnalyser per tracked player
  ...
  // Callbacks — keyed by jersey number
  ...
  Future<void> startCamera(List<String> targetJerseys) async {
    ...
    // Create a PoseAnalyser for each player
  ...
  Future<void> _processFrame(CameraImage image) async {
    ...
    try {
      ...
      // Pose detection — assign poses to nearest tracked player
      ...
      for (final jersey in _targetJerseys) {
        if ((_framesSinceDetection[jersey] ?? 0) > _maxFramesWithoutDetection) {
          ...
          // Hold last known position briefly
  ...
  /// Assign each detected pose to the closest tracked player bounding box
  void _assignPosesToPlayers(List<Pose> poses) {
    for (final pose in poses) {
      // Get approximate centre of this pose from hip landmarks
      ...
      // Find which tracked player's box this pose centre falls inside
      ...
      for (final jersey in _targetJerseys) {
        ...
        // Check if pose centre is within this player's bounding box
        if (box.contains(Offset(poseCentreX, poseCentreY))) {
          final boxCentreX = box.left + box.width / 2;
          final boxCentreY = box.top  + box.height / 2;
          final dist = _distance(poseCentreX, poseCentreY, boxCentreX, boxCentreY);
      ...
      // If no box match, assign to nearest player
  ...
  Future<void> _runOCR(InputImage inputImage) async {
    ...
    // Check each detected text block against ALL target jerseys
